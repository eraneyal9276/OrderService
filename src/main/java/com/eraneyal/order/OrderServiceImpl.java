...
import com.eraneyal.order.proto.Item;
...
// -- validate input
        ...
        Map<String,OrderItem> items = getOrderItems (in);
...
// -- validate input
        ...
        logger.info ("trackingUpdate {}-{}-{}", in.getOrderId (), in.getAllocationId (), in.getStatus ());
        EntityRef<Order.Command>
            entityRef = sharding.entityRefFor (Order.ENTITY_KEY, in.getOrderId ());
            ...
            reply.thenApply (
                order -> {
                    if (order.allocations ().isEmpty ())
                        ...
                        return toProtoOrder (in.getOrderId (), order);
                });
...
/**
  ...
  */
    ...
    private static Map<String,OrderItem> getOrderItems (ReceiveOrderRequest request)
    {
        Map<String,OrderItem> items = new HashMap<> ();

        try {
            for (int i = 0; i < request.getItemsCount (); i++) {
                Item item = request.getItems (i);
                items.put (
                    item.getItemId (),
                    new OrderItem (
                        item.getItemId (),
...
/**
  ...
  */
    ...
    private static List<Item> toProtoItems (Map<String,OrderItem> items)
    {
        return items.values ()
                    .stream ()
...
/**
  ...
  */
    ...
    private static List<com.eraneyal.order.proto.Status> toProtoStatuses (
        Map<Instant,Allocation.Status> statuses)
    {
...
/**
  ...
  * <p>
  * @param ident the order identifier
  * @param details the order details
  * @return the proto order
  */

    private static OrderDetailsResponse toProtoOrder (
        String ident,
